#!/usr/bin/env bash
# Common helpers for lvm-lab test suite
...
SERVER_PORT="$(_get_port lvm-lab)"
if [[ -z "$SERVER_PORT" ]]; then echo "ERROR: Cannot find VM port. Is lvm-lab running?"; exit 1; fi
...
clean_lvm() {
    log_info "Cleaning up LVM test artifacts..."
    ssh_vm "sudo umount /mnt/lvm /mnt/xfs /mnt/snap 2>/dev/null; sudo lvremove -f myvg 2>/dev/null; sudo vgremove -f myvg 2>/dev/null; sudo pvremove -ff /dev/vdb /dev/vdc /dev/vdd /dev/vde 2>/dev/null; sudo wipefs -a /dev/vdb /dev/vdc /dev/vdd /dev/vde 2>/dev/null; true" 2>/dev/null || true
}
